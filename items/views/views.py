...
class ItemDetailView(PermissionRequiredMixin, DetailView):
    ...
    def get_permission_object(self):
        return self.get_object().lot.catalogue.collection_tmp.dataset
...
class ItemAuthorDetailView(PermissionRequiredMixin, DetailView):
    ...
    def get_permission_object(self):
        return self.get_object().item.lot.catalogue.collection_tmp.dataset
    # End permission check
        ...
        if 'entries' in request.POST:
            ...
            for item_id in items:
                ...
                if form_set.is_valid() and 'change_dataset' in \
                        get_perms(request.user, item.lot.catalogue.collection_tmp.dataset):
    ...
    if request.method == 'POST':
        if 'entries' in request.POST:
            ...
            if publicationplaceform.is_valid():
                for item_id in items:
                    item = Item.objects.get(uuid=item_id)
                    if 'change_dataset' in get_perms(request.user, item.lot.catalogue.collection_tmp.dataset):
                        if not item.edition:
